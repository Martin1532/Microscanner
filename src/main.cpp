...
void inputSerial()
{
   ...
   else if (readSerial == "RD")
  {
    ...
    else if (readSerial == "PN")
    {
        zDevault(z_pas);
    }

    else if (readSerial == "PF")
    {
        z_backtohome();
    }
    ...
    {
        end[0] = koorXYZ[0]; // koordinat x end
        end[1] = koorXYZ[1]; // koordinat y end
...
void run()
{
    ...
    {
        if (XX == 1 && currkoorX <= row && totalImg <= img)
        {
        ...
        if (XX == 0 && currkoorX >= 1 && totalImg <= img)
        {
            delay(delayAfterCapture);
            Serial.print("V");
            Serial.print(";");
