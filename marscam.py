...
class marscam:
    ...
    def subscribeCameraStatus(self, camera):        
        ...
        self.nRet = GENICAM_createEventSubscribe(byref(eventSubscribeInfo), byref(eventSubscribe))
        if ( self.nRet != 0):
            print("create eventSubscribe fail!")
            return -1
        
        self.nRet = eventSubscribe.contents.subscribeConnectArgsEx(eventSubscribe, connectCallBackFuncEx, g_cameraStatusUserInfo)
        if ( self.nRet != 0 ):
    ...
    def unsubscribeCameraStatus(self, camera):        
        ...
        self.nRet = eventSubscribe.contents.unsubscribeConnectArgsEx(eventSubscribe, connectCallBackFuncEx, g_cameraStatusUserInfo)
        if ( self.nRet != 0 ):
    ...
    def setSoftTriggerConf(self, camera):        
        ...
        self.nRet = GENICAM_createAcquisitionControl(pointer(acqCtrlInfo), byref(acqCtrl))
        if ( self.nRet != 0 ):
        ...
        self.nRet = trigSourceEnumNode.setValueBySymbol(byref(trigSourceEnumNode), b"Software")
        if ( self.nRet != 0 ):
        ...
        self.nRet = trigSelectorEnumNode.setValueBySymbol(byref(trigSelectorEnumNode), b"FrameStart")
        if ( self.nRet != 0 ):
        ...
        self.nRet = self.trigModeEnumNode.setValueBySymbol(byref(self.trigModeEnumNode), b"On")
        if ( self.nRet != 0 ):
    ...
    def setLineTriggerConf(self, camera):
        ...
        trigSourceEnumNode = acqCtrl.contents.triggerSource(acqCtrl)
        self.nRet = trigSourceEnumNode.setValueBySymbol(byref(trigSourceEnumNode), b"Line1")
        if ( self.nRet != 0 ):
        ...
        self.nRet = trigActivationEnumNode.setValueBySymbol(byref(trigActivationEnumNode), b"RisingEdge")
        if ( self.nRet != 0 ):
    ...
    def _openCamera(self, camera):
        # connect camera
        self.nRet = camera.connect(camera, c_int(GENICAM_ECameraAccessPermission.accessPermissionControl))
        if ( self.nRet != 0 ):
    ...
    def setExposureTime(self, dVal):
        ...
        self.nRet = GENICAM_createDoubleNode(byref(exposureTimeNodeInfo), byref(exposureTimeNode))
        if ( self.nRet != 0 ):
            print("create ExposureTime Node fail!")
            return -1
        
        # set ExposureTime
        self.nRet = exposureTimeNode.contents.setValue(exposureTimeNode, c_double(dVal))  
        if ( self.nRet != 0 ):
    ...
    def setBrightness(self, dVal):
        ...
        self.nRet = GENICAM_createIntNode(byref(brightnessNodeInfo), byref(brightnessNode))
        if ( self.nRet != 0 ):
            print("create brightness Node fail!")
            return -1
        
        # set brightness
        self.nRet = brightnessNode.contents.setValue(brightnessNode, c_longlong(dVal))
        if ( self.nRet != 0 ):
    ...
    def setExposureAuto(self, mode="Off"):        
        ...
        self.nRet = exposureAutoEnumNode.setValueBySymbol(byref(exposureAutoEnumNode), bytes(mode, 'utf-8'))
        if ( self.nRet != 0 ):
            print("set TriggerSource value [Software] fail!")
            # release node resource before return
    ...
    def setWBAuto(self, mode="Off"):        
        ...
        self.nRet = GENICAM_createAnalogControl(pointer(acqCtrlInfo), byref(acqCtrl))
        if ( self.nRet != 0 ):
        ...
        self.nRet = balanceWhiteAutoEnumNode.setValueBySymbol(byref(balanceWhiteAutoEnumNode), bytes(mode, 'utf-8'))
        if ( self.nRet != 0 ):
            print("set TriggerSource value [Software] fail!")
            # release node resource before return
    ...
    def setWBRatio(self, channel, dVal):        
        ...
        self.nRet = GENICAM_createAnalogControl(pointer(anlgctrlInfo), byref(anlgctrl))
        if ( self.nRet != 0 ):
            print("create AcquisitionControl fail!")
            return -1
        ...
        self.nRet = balSelectorEnumNode.setValueBySymbol(byref(balSelectorEnumNode), bytes(channel, 'utf-8'))
        if ( self.nRet != 0 ):
            print("set TriggerSelector value [FrameStart] fail!")
            # release node resource before return
        ...
        self.nRet = GENICAM_createDoubleNode(byref(balRatioNodeInfo), byref(balRatioNode))
        if ( self.nRet != 0 ):
            print("create balRatio Node fail!")
            return -1
        
        # set balRatio
        self.nRet = balRatioNode.contents.setValue(balRatioNode, c_double(dVal))  
        if ( self.nRet != 0 ):
    ...
    def enumCameras(self):
        ...
        cameraCnt = c_uint()
        self.nRet = system.contents.discovery(system, byref(cameraList), byref(cameraCnt), c_int(GENICAM_EProtocolType.typeAll));
        if ( self.nRet != 0 ):
    ...
    def grabOne(self, camera):
        ...
        self.nRet = GENICAM_createStreamSource(pointer(self.streamSourceInfo), byref(self.streamSource))
        if ( self.nRet != 0 ):
    ...
    def setROI(self, camera, OffsetX, OffsetY, nWidth, nHeight):
        ...
        self.nRet = GENICAM_createIntNode(byref(widthMaxNodeInfo), byref(widthMaxNode))
        if ( self.nRet != 0 ):
            print("create WidthMax Node fail!")
            return -1
        
        oriWidth = c_longlong()
        self.nRet = widthMaxNode.contents.getValue(widthMaxNode, byref(oriWidth))
        if ( self.nRet != 0 ):
        ...
        self.nRet = GENICAM_createIntNode(byref(heightMaxNodeInfo), byref(heightMaxNode))
        if ( self.nRet != 0 ):
            print("create HeightMax Node fail!")
            return -1
        
        oriHeight = c_longlong()
        self.nRet = heightMaxNode.contents.getValue(heightMaxNode, byref(oriHeight))
        if ( self.nRet != 0 ):
        ...
        self.nRet = GENICAM_createIntNode(byref(widthNodeInfo), byref(widthNode))
        if ( self.nRet != 0 ):
        ...
        self.nRet = GENICAM_createIntNode(byref(heightNodeInfo), byref(heightNode))
        if ( self.nRet != 0 ):
            print("create Height Node fail!")
            return -1
        
        self.nRet = heightNode.contents.setValue(heightNode, c_longlong(nHeight))
        if ( self.nRet != 0 ):
        ...
        self.nRet = GENICAM_createIntNode(byref(OffsetXNodeInfo), byref(OffsetXNode))
        if ( self.nRet != 0 ):
            print("create OffsetX Node fail!")
            return -1
        
        self.nRet = OffsetXNode.contents.setValue(OffsetXNode, c_longlong(OffsetX))
        if ( self.nRet != 0 ):
        ...
        self.nRet = GENICAM_createIntNode(byref(OffsetYNodeInfo), byref(OffsetYNode))
        if ( self.nRet != 0 ):
            print("create OffsetY Node fail!")
            return -1
        
        self.nRet = OffsetYNode.contents.setValue(OffsetYNode, c_longlong(OffsetY))
        if ( self.nRet != 0 ):
    ...
    def openCamera(self):
        ...
        self.nRet = GENICAM_createEnumNode(byref(self.trigModeEnumNodeInfo), byref(self.trigModeEnumNode))
        if ( self.nRet != 0 ):
        ...
        self.nRet = self.trigModeEnumNode.contents.setValueBySymbol(self.trigModeEnumNode, b"Off")
        if ( self.nRet != 0 ):
    ...
    def setDoubleNode(self, attr, dVal):
        ...
        self.nRet = GENICAM_createDoubleNode(byref(doubleNodeInfo), byref(doubleNode))
        if ( self.nRet != 0 ):
    ...
    def setSharpnessEnable(self, mode="Off"):        
        ...
        self.nRet = GENICAM_createISPControl(pointer(acqCtrlInfo), byref(acqCtrl))
        if ( self.nRet != 0 ):
        ...
        self.nRet = sharpnessEnableEnumNode.setValueBySymbol(byref(sharpnessEnableEnumNode), bytes(mode, 'utf-8'))
        if ( self.nRet != 0 ):
            print("set TriggerSource value [Software] fail!")
            # release node resource before return
